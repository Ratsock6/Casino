...
export const execute = async (interaction: ChatInputCommandInteraction) => {
  ...
  try {
    ...
    const categoryId = process.env.DISCORD_CREDIT_CATEGORY_ID;
    ...
    const channelName = `credit-${user.username.toLowerCase()}-${Date.now().toString().slice(-4)}`;
    ...
    const resumeEmbed = new EmbedBuilder()
      .setTitle('💸 Demande de crédit — Bellagio Casino')
      .setColor(0xC9A84C)
      .setDescription(`Bonjour <@${interaction.user.id}> ! Un membre du staff va prendre en charge votre demande de crédit. Merci de patienter.`)
      .addFields(
        ...
        { name: '\u200b', value: '\u200b', inline: true },
        {
          name: '📋 Instructions', value: [
            ...
            '• Le crédit se fait **en jeu, en safe zone**',
    ...
    await channel.send({
      content: `${staffMention} — Nouvelle demande de crédit de <@${interaction.user.id}>`,
      embeds: [resumeEmbed],
    });
